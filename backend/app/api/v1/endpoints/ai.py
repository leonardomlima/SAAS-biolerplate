...
async def create_agent(
    ...
    agent = await agent_crud.create(
        db, obj_in=agent_data, tenant_id=current_user.tenant_id
    )
...
async def get_agent(
    ...
    agent = await agent_crud.get_by_id(
...
async def delete_agent(
    ...
    """Deleta um agente"""
    success = await agent_crud.delete(
        db, agent_id=agent_id, tenant_id=current_user.tenant_id
    )
    if not success:
        raise HTTPException(
...
async def send_chat_message(
    ...
    # Obter agente
    agent = await agent_crud.get_by_id(
        db, agent_id=data.agent_id, tenant_id=current_user.tenant_id
    )
    if not agent:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Agente não encontrado"
    ...
    if data.session_id:
        session = await session_crud.get_by_id(
            db, session_id=data.session_id, tenant_id=current_user.tenant_id
        )
        if not session:
            raise HTTPException(
    ...
    history = await message_crud.get_session_messages(
        db, session_id=session.id, tenant_id=current_user.tenant_id, limit=20
    )
    ...
    response = await llm.chat_completion(
        ...
        model_name=agent.model_name,
        temperature=agent.temperature,
        reasoning_effort=agent.reasoning_effort.value
...
async def get_session_messages(
    ...
    messages = await message_crud.get_session_messages(
        db, session_id=session_id, tenant_id=current_user.tenant_id, limit=100
    )
    return [
        {
            "id": m.id,
...
async def delete_chat_session(
    ...
    success = await session_crud.delete(
        db, session_id=session_id, tenant_id=current_user.tenant_id
    )
    if not success:
        raise HTTPException(
...
"""
    ...
    schema_context = "\n".join(
        [f"Tabela {table}: {cols}" for table, cols in table_schemas.items()]
    )
